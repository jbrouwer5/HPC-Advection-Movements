...
void lax_distributed(int N, double L, double T, int nprocs, int mype)
{
    ...
    if (sqrt(nprocs) == int(sqrt(nprocs))) {
        ...
        printf("process: %d has neigbors up:%d, down:%d, left:%d, right:%d\n", mype, u_neighbor, d_neighbor, l_neighbor, r_neighbor);
    } 
    else {
        return;
    }
    ...
    for (double k=0; k<T; k+=d_t) // iterate T with delta t step size
    {
        ...
        for (int i=1; i<num_rows+1; i++)
        {
            ...
            send_cols[2][i-1] = current[(num_rows)*(full_cols)+i]; // down row 
        ...
        for (int i=1; i<num_cols+1; i++)
        {
            ...
            current[(num_rows+1)*(full_cols)+i] = ghost_cells[2][i-1]; // down
        }
        ...
        for (int i=1; i<num_rows+1; i++) // for every cell i,j
        {
            ...
            {
                next[i*(num_cols+2)+j] =  (current[(i+1)*(full_cols)+j] + 
